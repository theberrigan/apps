...
export class PaymentsService {
    ...
    public getPaymentMethods (quoteCode : string) : Promise<any> {
        return this.http.get({
            url: 'endpoint://payment.get_payment_methods',
            params: { quoteCode }
        })
        .toPromise()
        .then((response : any) => response)
        .catch((reason : any) => {
            console.warn(`[PaymentsService.getPaymentMethods]`, reason);
            return null;
        });
    }

    public paySofort (quoteCode : string, amount : number) : Promise<any> {
        return this.http.post({
            url: 'endpoint://payment.get_sofort_quote_url',
            params: { quoteCode },
            body: { amount }
        })
            .toPromise()
            .then((response : any) => response)
            .catch((reason : any) => {
                console.warn(`[PaymentsService.paySofort]`, reason);
                return null;
            });
    }

    public saveStripeToken (quoteCode : string, source : string, amount : number) : Promise<boolean> {
        return this.http.post({
            url: 'endpoint://payment.save_stripe_token',
            params: { quoteCode },
            body: { source, amount }
        })
            .toPromise()
            .then(() => true)
            .catch((reason : any) => {
                console.warn(`[PaymentsService.saveStripeToken]`, reason);
                return false;
            });
    }
