...
import {cloneDate, defer, int, isEmpty, isFinite} from '../../lib/utils';
...
export class DatepickerComponent implements OnInit, OnDestroy, OnChanges, AfterViewInit, ControlValueAccessor {
    ...
    @ViewChild('popup', { read: PopupComponent })
    ...
    public parseOptions () : void {
        ...
        this._format = (this.format || '').trim() || 'YYYY-MM-DD';
    ...
    public updateInputFromMainModel () : void {
        this.inputModel = this.mainModel && this.datetimeService.getMoment(this.mainModel).format(this.format) || '';
        // console.warn('moment -> input', this.inputModel);
    }

    public updateMainModelFromInput () : void {
        const moment  = this.datetimeService.parse((this.inputModel || '').trim(), this.format);
        this.mainModel = moment && moment.toDate();
    ...
    public onDocumentClick (e : any) {
        if (this.isLastClickByBubble || !this.activePicker) {
            return;
        }

        const isToggleClick = this.domService.hasEventMark(e, 'dtPickerToggleClick');
        const isBubbleClick = this.domService.hasEventMark(e, 'dtPickerBubbleClick');
    ...
    public onToggleClick (e : any) : void {
        this.domService.markEvent(e, 'dtPickerToggleClick');
        this.activePicker ? this.deactivate() : this.activate();
    }
    ...
    public setPopupState (isActive : boolean) : void {
        if (!this.popup) {
            return;
        }

        if (isActive) {
            this.popupModel = this.getPopupInitialDate();
            ...
            this.popup && this.popup.activate();
            this.activePicker = 'popup';
        } else {
            this.activePicker = null;
            this.popup && this.popup.deactivate();
            this.renderer.removeClass(document.body, 'datepicker-no-scroll');
            this.popupModel = null;
            this.popupState.type = null;
            this.popupState.prevY = 0;
            forIn(this.popupState.displays, display => {
                display.items = [];
                display.offset = 0;
            });
            if (this.popupListeners) {
    ...
    public getPopupInitialDate () : Date {
    ...
    public generatePopupData () : void {
        ...
        this.popupState.displays.date.items = range(1, this.getDaysInMonth(model) + 1).map((value, index) => {
            if (value === modelDate) {
                this.popupState.displays.date.offset = -1 * POPUP_ITEM_HEIGHT * index;
            }
        ...
        this.popupState.displays.month.items = this.datetimeService.getShortMonths().map((value, index) => {
            if (index === modelMonth) {
                this.popupState.displays.month.offset = -1 * POPUP_ITEM_HEIGHT * index;
            }
        ...
        this.popupState.displays.year.items = range(modelYear - 10, modelYear + 10).map((value, index) => {
            if (value === modelYear) {
                this.popupState.displays.year.offset = -1 * POPUP_ITEM_HEIGHT * index;
            }
        ...
        if (this.hasTimePicker) {
            ...
            this.popupState.displays.timeSuffix.items = [ 'AM', 'PM' ].map((value, index) => {
                if (value === modelSuffix) {
                    this.popupState.displays.timeSuffix.offset = -1 * POPUP_ITEM_HEIGHT * index;
                }
            ...
            this.popupState.displays.hours.items = range(startHour, endHour).map((value, index) => {
                if (value === modelHours) {
                    this.popupState.displays.hours.offset = -1 * POPUP_ITEM_HEIGHT * index;
                }
            ...
            this.popupState.displays.minutes.items = range(0, 60, 5).map((value, index) => {
                if (value === modelMinutes) {
                    this.popupState.displays.minutes.offset = -1 * POPUP_ITEM_HEIGHT * index;
                }
        ...
        forIn(this.popupState.displays, (_, key) => {
    ...
    public updatePopupItems (type : string) : void {
        const offsetCount = Math.round(this.popupState.displays[type].offset / POPUP_ITEM_HEIGHT);
        const items = this.popupState.displays[type].items;
        ...
        if (type === 'timeSuffix') {
            ...
            if (type === 'year') {
                ...
                if (offsetCount >= -1) {
                    this.popupState.displays.year.items = [ ...newItems, ...items.slice(0, -10) ];
                    this.popupState.displays.year.offset -= POPUP_ITEM_HEIGHT * 10;
                } else {
                    this.popupState.displays.year.items = [ ...items.slice(10), ...newItems ];
                    this.popupState.displays.year.offset += POPUP_ITEM_HEIGHT * 10;
                }
            } else {
                if (offsetCount >= -1) {
                ...
                this.popupState.displays[type].items = [ ...items.slice(replaceCount), ...items.slice(0, replaceCount) ];
                this.popupState.displays[type].offset += replaceCount * POPUP_ITEM_HEIGHT;
            }
        }
    }

    public onPopupTouchStart (e : any, type : string) : void {
        // console.log('start');
        this.popupState.type = type;
        this.popupState.prevY = e.touches[0].clientY;
    }

    public onPopupTouchMove (e : any) : void {
        if (!this.popupState.type) {
            return;
        }

        const { type, prevY } = this.popupState;
        const currentY = e.touches[0].clientY;
        const deltaY = prevY - currentY;
        let newOffset = this.popupState.displays[type].offset - deltaY;
        newOffset = type === 'timeSuffix' ? Math.max(-1 * POPUP_ITEM_HEIGHT, Math.min(0, newOffset)) : newOffset;
        ...
        this.popupState.prevY = currentY;
        this.popupState.displays[type].offset = newOffset;

        this.updatePopupItems(type);
    }

    public onPopupTouchEnd (e : any) : void {
        // console.log(e.type);
        const { type } = this.popupState;

        if (type) {
            this.popupState.type = null;
            this.popupState.displays[type].offset = Math.round(this.popupState.displays[type].offset / POPUP_ITEM_HEIGHT) * POPUP_ITEM_HEIGHT;

            // -----------------
            const displays = this.popupState.displays;

            let date = displays.date.items.find(d => d.isActive).value;
            const month = displays.month.items.find(m => m.isActive).index;
            const year = displays.year.items.find(y => y.isActive).value;
            let hours : number = 0;
            let minutes : number = 0;

            if (this.hasTimePicker) {
                hours = displays.hours.items.find(h => h.isActive).value;
                minutes = displays.minutes.items.find(m => m.isActive).value;
            ...
            if (type === 'year' && month === 1 || type === 'month') {
            ...
            this.popupModel = new Date(year, month, date, hours, minutes, 0, 0);
    ...
    public onBubbleClick (e : any) : void {
        this.domService.markEvent(e, 'dtPickerBubbleClick');
    }
    ...
    public onMouseDown (e : any) : void {
        this.isLastClickByBubble = this.domService.hasEventMark(e, 'dtPickerBubbleMouseDown');
    }
