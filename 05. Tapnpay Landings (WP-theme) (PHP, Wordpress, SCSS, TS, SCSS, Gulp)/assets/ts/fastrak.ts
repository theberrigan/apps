...
const injectFormSubmittedTag = async () => {
    return new Promise<void>((resolve) => {
        ...
        script.src = 'https://tag.simpli.fi/sifitag/12fe8960-5a18-013a-c28a-06a60fe5fe77';
...
const init = () => {
    const config : AppConfig = (<any>window).fastrakConfig;

    if (!config) {
        throw new Error('App config is not found');
    }

    (<any>window).fastrak = new Application(config);
};
