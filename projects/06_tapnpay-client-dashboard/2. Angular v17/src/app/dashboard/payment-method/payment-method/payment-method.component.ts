...
export class PaymentMethodComponent implements OnInit, OnDestroy {
    ...
    @Output() onPaymentMethodSelected = new EventEmitter<any>();
    ...
    cardNumberStripeElement: any = null;

    cardNumberInputState: any = null;

    hasCardNumberFocus: boolean = false;

    cardExpireStripeElement: any = null;

    cardExpireDateInputState: any = null;

    hasCardExpireFocus: boolean = false;

    cardCVCStripeElement: any = null;
    ...
    methodSelectDialogStates: 'init' | 'ready' | 'error';
    ...
    @Input() subscriptionPaymentError: Error = null;
    isAutoPaymentEnabledFormValue: boolean = false;
    isAutoPaymentEnabledServerValue: boolean = false;
    ...
    ngOnDestroy(): void {
        this.resetSubscriptionPaymentError();
    ...
    async init() {
        ...
        this.autoPaymentService.checkAutopaymentStatus().subscribe((response) => {
            this.isAutoPaymentEnabledServerValue = response.approvalStatus;
            if (this.isAutoPaymentEnabledFormValue !== this.isAutoPaymentEnabledServerValue) {
                this.isAutoPaymentEnabledFormValue = this.isAutoPaymentEnabledServerValue;
            }
        });
        ...
        if (this.isSilentInit) {
            ...
            this.methodSelectDialogStates = 'init';
            this.activePopup = 'method';
            this.onShow.emit();
    ...
    async initMethodPopup(): Promise<void> {
        return new Promise((resolve) => {
            ...
            Promise.all([
                firstValueFrom(this.paymentService.fetchPaymentOptions()),
                firstValueFrom(this.paymentService.fetchPaymentConfig()),
                this.stripeService.getMobileWalletPaymentRequest({
                    amount: 50,
                    currency: 'usd'
                })
                ...
                this.methodSelectDialogStates = 'ready';
            }).catch((e) => {
                console.warn('Error initMethodPopup:', e);
                this.methodSelectDialogStates = 'error';
            }).then(() => {
                resolve();
            });
        });
    ...
    updatePaymentOptions(paymentOptions: null | PaymentOptions) {
        ...
        const CARD_OPTION_LABEL_KEYS = {
            both: 'payment_method.options.card',
            debit: 'payment_method.options.card_debit',
            credit: 'payment_method.options.card_credit'
        };

        const ADD_CARD_BUTTON_CAPTION_KEYS = {
            both: 'payment_method.add_card',
            debit: 'payment_method.add_card_debit',
            credit: 'payment_method.add_card_credit'
        };

        const CARD_POPUP_HEADER_KEYS = {
            both: 'payment_method.card.card_header',
            debit: 'payment_method.card.card_debit_header',
            credit: 'payment_method.card.card_credit_header'
        };

        const CARD_TYPE_ERROR_KEYS = {
            both: 'payment_method.error.credit_and_debit_only',
            debit: 'payment_method.error.debit_only',
            credit: 'payment_method.error.credit_only'
        };

        let cardState: 'both' | 'debit' | 'credit' | null = null;
        ...
        this.cardOptionLabelKey = cardState ? CARD_OPTION_LABEL_KEYS[cardState] : null;
        this.addCardButtonCaptionKey = cardState ? ADD_CARD_BUTTON_CAPTION_KEYS[cardState] : null;
        this.cardPopupHeaderKey = cardState ? CARD_POPUP_HEADER_KEYS[cardState] : null;
        this.cardTypeErrorKey = cardState ? CARD_TYPE_ERROR_KEYS[cardState] : null;
    }
    ...
    async setupStripeCardFormInputElements() {
        const elements: Elements = await this.stripeService.createElements();

        return Promise.all([
            new Promise<void>((resolve) => {
                this.cardNumberStripeElement = elements.create('cardNumber', {
                    style: this.stripeInputStyle,
                    classes: this.stripeInputClasses
                });
                this.cardNumberStripeElement.on('change', (event: any) => this.cardNumberInputState = event);
                this.cardNumberStripeElement.on('ready', () => resolve());
                this.cardNumberStripeElement.on('focus', () => this.hasCardNumberFocus = true);
                this.cardNumberStripeElement.on('blur', () => this.hasCardNumberFocus = false);
                this.cardNumberStripeElement.mount(this.cardNumberNest.nativeElement);
            }),
            new Promise<void>((resolve) => {
                this.cardExpireStripeElement = elements.create('cardExpiry', {
                    style: this.stripeInputStyle,
                    classes: this.stripeInputClasses
                });
                this.cardExpireStripeElement.on('change', (event: any) => this.cardExpireDateInputState = event);
                this.cardExpireStripeElement.on('ready', () => resolve());
                this.cardExpireStripeElement.on('focus', () => this.hasCardExpireFocus = true);
                this.cardExpireStripeElement.on('blur', () => this.hasCardExpireFocus = false);
                this.cardExpireStripeElement.mount(this.cardExpireNest.nativeElement);

            }),
            new Promise<void>((resolve) => {
                this.cardCVCStripeElement = elements.create('cardCvc', {
                    style: this.stripeInputStyle,
                    classes: this.stripeInputClasses
                });
                this.cardCVCStripeElement.on('change', (event: any) => this.cardCVCState = event);
                this.cardCVCStripeElement.on('ready', () => resolve());
                this.cardCVCStripeElement.on('focus', () => this.hasCardCVCFocus = true);
                this.cardCVCStripeElement.on('blur', () => this.hasCardCVCFocus = false);
                this.cardCVCStripeElement.mount(this.cardCVCNest.nativeElement);
            })
        ]);
    }
    ...
    onNewCardClick() {
        ...
        defer(() => {
            ...
            this.setupStripeCardFormInputElements().then(() => {
                this.resetCard();
    ...
    setCardPopupSubmitting(isSubmitting: boolean) {
        ...
        if (this.cardNumberStripeElement) {
            this.cardNumberStripeElement.update({disabled: isSubmitting});
        }

        if (this.cardExpireStripeElement) {
            this.cardExpireStripeElement.update({disabled: isSubmitting});
        }

        if (this.cardCVCStripeElement) {
            this.cardCVCStripeElement.update({disabled: isSubmitting});
        }
    }
    ...
    async onSubmitCard() {
        ...
        const setupCardResult: SetupIntentResponse = await this.stripe.confirmCardSetup(clientSecret, {
            payment_method: {
                card: this.cardNumberStripeElement,
            },
        }).catch(() => null);
    ...
    private destroyCard() {
        ...
        if (this.cardNumberStripeElement) {
            this.cardNumberStripeElement.destroy();
            this.cardNumberStripeElement = null;
        }

        if (this.cardExpireStripeElement) {
            this.cardExpireStripeElement.destroy();
            this.cardExpireStripeElement = null;
        }

        if (this.cardCVCStripeElement) {
            this.cardCVCStripeElement.destroy();
            this.cardCVCStripeElement = null;
        }

        this.cardNumberInputState = null;
        this.cardExpireDateInputState = null;
    ...
    resetCard() {
        ...
        if (this.cardNumberStripeElement) {
            this.cardNumberStripeElement.clear();
        }

        if (this.cardExpireStripeElement) {
            this.cardExpireStripeElement.clear();
        }

        if (this.cardCVCStripeElement) {
            this.cardCVCStripeElement.clear();
        }
    }
    ...
    isCardValid(): boolean {
        return !!(
            ...
            this.cardNumberInputState &&
            !this.cardNumberInputState.error &&
            this.cardNumberInputState.complete &&
            this.cardExpireDateInputState &&
            !this.cardExpireDateInputState.error &&
            this.cardExpireDateInputState.complete &&
            this.cardCVCState &&
    ...
    onDCBMethodClick() {
        ...
        if (status === 'UNKNOWN') {
            ...
            this.setSelectedPaymentMethod('DCB');
        }
    }

    setSelectedPaymentMethod(methodType: PaymentMethodType, methodId: null | string = null, e: any = null) {

        const isDCBMethodIneligible = methodType === 'DCB' && (['UNKNOWN', 'INELIGIBLE'].includes(this.paymentOptions.DCB.status));
        const isCardTypeDisabled = CARD_TYPES.includes(<PaymentMethodCard>methodType) && !this.paymentOptions[methodType].enabled;
        const isWalletMethodUnavailable = ['GOOGLEPAY', 'APPLEPAY'].includes(methodType) && (!this.paymentOptions.WALLET.enabled || !this.wallets || !this.wallets[methodType]);
        const isEventMarked = e && (this.domService.isHasEventMark(e, 'cardDeleteClick') || this.domService.isHasEventMark(e, 'venmoActionClick'));

        if (
            this.isMethodPopupSubmitting ||
            this.isCheckingDCBEligibility ||
            isDCBMethodIneligible ||
            isCardTypeDisabled ||
            isWalletMethodUnavailable ||
            isEventMarked
        ) {
            return;
        }
    ...
    async onSubmitMethod() {
        ...
        if (this.isSelectedPaymentMethod('VENMO') && !this.selectedPaymentMethod.id) {
        ...
        const paymentConfig: PaymentConfig = await firstValueFrom(this.paymentService.setNewPaymentMethod({
        ...
        if (paymentConfig) {
            ...
            const paymentOptions = await firstValueFrom(this.paymentService.fetchPaymentOptions()).catch(() => null);
    ...
    private setAutoPaymentOption() {
        const isCurrentPaymentMethodSupportAutoPayment = !this.autoPaymentService.disabledPaymentMethods.includes(this.selectedPaymentMethod.type) &&
            (this.mode !== 'edit' || (!this.isSelectedPaymentMethod("APPLEPAY") && !this.isSelectedPaymentMethod('GOOGLEPAY')));

        const isFormValueDifferentFromServer = this.isAutoPaymentEnabledFormValue !== this.isAutoPaymentEnabledServerValue;


        if (isFormValueDifferentFromServer && isCurrentPaymentMethodSupportAutoPayment) {
            if (this.isAutoPaymentEnabledFormValue && !this.isAutoPaymentEnabledServerValue) {
                this.enableAutoPayment();
            } else if (!this.isAutoPaymentEnabledFormValue && this.isAutoPaymentEnabledServerValue) {
                this.disableAutoPayment();
            }
        }

        if (!isCurrentPaymentMethodSupportAutoPayment && this.isAutoPaymentEnabledServerValue) {
            this.disableAutoPayment();
        }
    }
    ...
    async onDetachVenmo(e: MouseEvent) {
        ...
        if (
            this.isMethodPopupSubmitting ||
            this.isCheckingDCBEligibility ||
            this.isSelectedPaymentMethod('VENMO') ||
    ...
    async onSubmitDeleteVenmo() {
        ...
        const paymentConfig = await firstValueFrom(this.paymentService.detachVenmoPaymentMethod()).catch(() => null);
    ...
    onCancelMethod() {
        if (this.mode === 'setup' && this.methodSelectDialogStates !== 'error') {
            return;
        }
    ...
    updateSelectedPaymentMethod(selectCardId?: string) {
        ...
        if (selectCardId) {
            ...
            const isCardItemEnabled = cardItem && this.paymentOptions[cardItem.type].enabled;

            if (isCardItemEnabled) {
    ...
    async onCheckDCBEligibility() {
        ...
        const newDCBStatus: DCBStatus = await firstValueFrom(this.paymentService.checkDCBEligibility()).catch(() => null);
    ...
    resetSubscriptionPaymentError() {
        this.subscriptionPaymentError = null;
    }

    isShowDCBMessage() {
        return this.paymentConfig.payment_method_type === 'DCB';
    }

    private enableAutoPayment() {
        this.autoPaymentService.enableAutopayment().pipe(
            take(1)
        ).subscribe({
            next: () => {
                this.toastService.create({
    ...
    public isSelectedPaymentMethod(methodType: PaymentMethodType): boolean {
        return this.selectedPaymentMethod && this.selectedPaymentMethod.type === methodType;
    }

    isShowAutoPaymentCheckBox() {
        const isAutoPaymentNotDisabledForSelectedMethod = !this.autoPaymentService.disabledPaymentMethods.includes(this.selectedPaymentMethod.type) &&
            (this.mode !== 'edit' || (!this.isSelectedPaymentMethod("APPLEPAY") && !this.isSelectedPaymentMethod('GOOGLEPAY')));
        return this.selectedPaymentMethod?.type && isAutoPaymentNotDisabledForSelectedMethod;
    }

    isShowAutoPaymentNotSupportMessage() {
        return (this.autoPaymentService.disabledPaymentMethods.includes(this.selectedPaymentMethod.type)
                || (this.mode === 'edit' && (this.isSelectedPaymentMethod("APPLEPAY") || this.isSelectedPaymentMethod('GOOGLEPAY'))))
            && this.isAutoPaymentEnabledFormValue;
    }

    private disableAutoPayment() {
        this.autoPaymentService.disableAutopayment().pipe(
            take(1)
        ).subscribe({
            next: () => {
                this.toastService.create({
                    message: ['invoices_auto_payment.auto_payment_disabled'],
                    timeout: 1000
                });
            },
            error: () => {
                this.toastService.create({
                    message: ['invoices_auto_payment.auto_payment_disable_error'],
                    timeout: 1000
                });
            }
        });
