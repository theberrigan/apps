...
export class TermsInnerComponent implements OnInit, OnDestroy {
    ...
    async loadTerms () {
        ...
        let langCode = this.langService.getCurrentLangCode();
        ...
        if (this.isDashboard) {
            ...
            const name = this.acceptedTerms.terms_name;

            this.content = await this.termsService.fetchTerms(name, langCode).toPromise().catch(() => null);
            this.isAcceptable = false;
            this.state = this.content ? 'terms' : 'error';
            defer(() => this.redraw());
        } else {
            ...
            if (phone) {
                ...
                if (!this.isLangChangedManually && this.termsService.regLang && langCode != this.termsService.regLang) {
                    langCode = this.termsService.regLang;
