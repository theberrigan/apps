...
if __name__ == '__main__':
    ...
    for item in os.listdir(LOCALES_DIR):
        ...
        logLines = [ '{} ({} keys added):'.format(item, len(updatedKeys)) ]

        for key, value in updatedKeys.items():
            logLines.append('    ' + '{}:'.format(key).ljust(maxKeyLength) + ' ' + value)

        logText = '\n'.join(logLines)

        dateNow = datetime.datetime.now().strftime('%Y-%m-%d_%H-%M-%S')
        writeTextFile(os.path.join(LOCALES_DIR, f'sync_log_{ dateNow }.txt'), logText)
        print('\n' + logText)
