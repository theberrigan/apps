...
TypesMap = {
    ...
    0x253F2FE: 'FRAMENAME',
    0x253F2FA: 'COLLISION',
    0x253F2F6: 'MATSPECULAR',
    0x253F2F9: 'NIGHTCOLS',
    0x253F2FC: 'MATREFLECTION',
    0x253F2FD: 'MESHEXTENSION',
}
...
class ImportRenderware:
    ...
    def readSection(self, type, extra = None):
        ...
        if type >= 0 and header[0] != type:
            print('Expected type {}, found {}: {}'.format(TypesMap[type], TypesMap[header[0]] if header[0] in TypesMap else header[0], self.filename))
        ...
        elif DEBUG: print("Ignoring extension data of type " + hex(header[0]))
    ...
    def readSectionGeometry(self, header, index):
        ...
        print(texCount, vertCount, self.f.tell())
    ...
    def readSectionCollision(self, header, geometry):
        if not self.childrenOf or len(self.childrenOf[0]) == 0:
...
ImportRenderware('./img/slamvan.dff')