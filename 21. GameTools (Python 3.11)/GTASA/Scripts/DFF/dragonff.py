                            ...
                            if chunk.size > 0:
                                ...
                                while self.pos < _chunk_end:
                                    ...
                                    if chunk.type == types["Material Effects PLG"]:
                                        global _ext_count
                                        _ext_count+=chunk.size
                                        print(_ext_count)
                                        self.read_matfx(material, chunk)
                                        # print(self.pos, _chunk_end)
        ...
        if chunk.type == types["Struct"]:
            ...
            while self.pos < root_end-12:
                ...
                #in the same loop
                    ...
                    print('chunk.type', hex(chunk.type), chunk.size)
                    self.pos += chunk.size
...
if __name__ == '__main__':
    model = dff()
    model.load_file('./GTA_SA_files/models/gta3.img/bravura.dff')