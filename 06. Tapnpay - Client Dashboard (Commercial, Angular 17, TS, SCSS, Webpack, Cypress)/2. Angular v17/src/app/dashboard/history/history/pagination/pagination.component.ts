...
export class PaginationComponent implements OnInit, OnDestroy {
    ...
    public set data(pagination: Pagination) {
        if (pagination) {
            this.generatePaginationButtons(pagination);
    ...
    private generatePaginationButtons(pagination: Pagination): void {
        const buttons = [];
        const leftGapFrom = 1;
        ...
        for (let page = 0, max = pagination.total_pages; page < max; page++) {
            ...
            const isGap =
                (page >= leftGapFrom && page <= leftGapTo) ||
                (page >= rightGapFrom && page <= rightGapTo);
