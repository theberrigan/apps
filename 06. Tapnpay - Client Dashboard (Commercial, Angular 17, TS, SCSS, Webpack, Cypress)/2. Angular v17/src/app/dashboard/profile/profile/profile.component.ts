...
import {AccountStatus, UserService} from '../../../services/user.service';
...
export class ProfileComponent implements OnInit, OnDestroy {
    ...
    activePopup: 'deactivate-account' | 'deactivate-account-result' | 'payment-method' | 'extend-rental-lpn-period';
    ...
    constructor(
        ...
        window.scroll(0, 0);
        this.onAccountStatusChange(this.userService.getUserData().account.accountStatus)
        this.isFleetModel = this.userService.getUserData().account.paymentModel === 'FLEET';
    ...
    ngOnInit() {
        ...
        this.userService.refreshAccountStatus().subscribe( (resp) => {
            this.onAccountStatusChange(resp);
        })
    }
    ...
    onAccountStatusChange(accountStatus : AccountStatus) {
        this.isDebtLock = accountStatus === 'ACCOUNT_DEBT_LOCK';
    }
