...
export class DeviceService {
    ...
    constructor() {
        ...
        this.onResize = (() => {
            ...
            const update = () => {
                ...
                const viewportBreakpoint = this.getViewportBreakpoint(viewportWidth);
                const message: any = {};
        ...
        [
            ...
            // Others
            browser.bot = this.isBrowserBot(ua);
    ...
    private isBrowserBot(ua: string) {
        return /(yandex|google|stackrambler|aport|slurp|msnbot|bingbot|twitterbot|ia_archiver|facebookexternalhit)/i.test(ua);
    }

    private getBrowserVersion(ua: string) {
        return (ua.match(/.+(?:me|ox|on|rv|it|era|opr|ie|edge)[\/: ]([\d.]+)/) || [null, '0'])[1];
    }

    private getViewportBreakpoint(viewportWidth: number) {
        return viewportWidth < 576 ? 'mobile' : viewportWidth >= 992 ? 'desktop' : 'tablet';
    }
    ...
    public getScrollbarWidth(): number {
        ...
        if (typeof this.scrollbarWidth === 'number') {
            return this.scrollbarWidth;
        }

        const createDivWithStyle = (styles: { [key: string]: string }): HTMLDivElement => {
        ...
        const div1 = createDivWithStyle({
            ...commonStyles,
            overflow: 'scroll'
        });

        const div2 = createDivWithStyle({
            ...commonStyles,
            overflow: 'hidden'
        });
